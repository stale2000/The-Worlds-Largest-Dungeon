...
from a magic item, such as a Spell Scroll. Your fea-
...
Most spellcasting monsters donâ€™t change their lists 
...
always have prepared doesnâ€™t count against the 
...
follow a spellâ€™s name.
Spell Level
Every spell has a level from 0 to 9, which is indi-
cated in a spellâ€™s description. A spellâ€™s level is an 
indicator of how powerful it is. Cantripsâ€”simple 
spells that can be cast almost by roteâ€”are level 0. 
...
Spell slots are the main way a spellcasterâ€™s magical 
...
spellâ€™s level or higher, effectively â€œfillingâ€ a slot with 
the spell. Imagine a spell slot is a groove of a certain 
sizeâ€”small for a level 1 slot and larger for a high-
er-level spell. A level 1 spell fits into a slot of any 
size, but a level 2 spell fits only into a slot thatâ€™s at 
...
doesnâ€™t expend a spell slot. To cast a spell as a Rit-
...
higher level, as detailed in a spellâ€™s description.
School of Magic
...
EvocationChannels energy to create effects that 
...
Class Spell Lists
If a spell is on a classâ€™s spell list, the classâ€™s name 
appears in parentheses after the spellâ€™s school of 
magic. Some features add a spell to a characterâ€™s 
spell list even if the character isnâ€™t a member of a 
...
minute or more. A spellâ€™s Casting Time entry speci-
fies which of those is required.
One Spell with a Spell Slot per Turn
On a turn, you can expend only one spell slot to cast 
a spell. This rule means you canâ€™t, for example, cast 
...
in response to a trigger that is defined in the spellâ€™s 
...
Certain spellsâ€”including a spell cast as a Rit-
ualâ€”require more time to cast: minutes or even 
...
centration (see â€œRules Glossaryâ€) while you do so. If 
your Concentration is broken, the spell fails, but you 
donâ€™t expend a spell slot. To cast the spell again, you 
must start over.
Range
A spellâ€™s range indicates how far from the spell-
caster the spellâ€™s effect can originate, and the spellâ€™s 
...
Touch. The spellâ€™s effect originates on something, 
...
If a spell has movable effects, they arenâ€™t restricted 
by its range unless the spellâ€™s description says 
otherwise.
Components
A spellâ€™s components are physical requirements the 
spellcaster must meet to cast the spell. Each spellâ€™s 
description indicates whether it requires Verbal 
(V), Somatic (S), or Material (M) components. If the 
spellcaster canâ€™t provide one or more of a spellâ€™s 
components, the spellcaster canâ€™t cast the spell.
Verbal (V)
...
voice. The words themselves arenâ€™t the source of the 
spellâ€™s power; rather, the particular combination of 
sounds, with specific pitch and resonance, sets the 
threads of magic in motion. Thus, a creature who is 
gagged or in an area of magical silence canâ€™t cast a 
...
in a spellâ€™s casting, as specified in parentheses in 
the Components entry. These materials arenâ€™t con-
sumed by the spell unless the spellâ€™s description 
...
used to perform Somatic components, if any.
    If a spell doesnâ€™t consume its materials and 
doesnâ€™t specify a cost for them, a spellcaster can use 
a Component Pouch (see â€œEquipmentâ€) instead of 
...
(see â€œEquipmentâ€ for descriptions).
Duration
A spellâ€™s duration is the length of time the spell per-
sists after it is cast. A duration typically takes one of 
the following forms:
Concentration. A duration that requires Concen-
tration follows the Concentration rules (see â€œRules 
Glossaryâ€).
Instantaneous. An instantaneous duration means 
the spellâ€™s magic appears only for a moment and 
then disappears.
...
GMâ€™s purview. Whatever the effects, they typically 
...
more targets to be affected by the spellâ€™s magic. A 
spellâ€™s description says whether the spell targets 
...
it canâ€™t be behind Total Cover.
...
other than you.
 Areas of Effect. Some spells, such as Thunder-
wave, cover an area called an area of effect, which 
is defined in â€œRules Glossary.â€ The area determines 
...
though it didnâ€™t attempt one (giving no hint that the 
...
the same spell cast multiple times donâ€™t combine. 
Instead, the most potent effectâ€”such as the high-
est bonusâ€”from those castings applies while their 
...
the same target, that target gains the spellâ€™s benefit 
only once; the target doesnâ€™t receive two bonus dice. 
...
targetâ€™s Hit Point maximum and current Hit Points 
increase by 5 for the duration.
    Using a Higher-Level Spell Slot. Each targetâ€™s Hit 
...
designate creatures that wonâ€™t set off the alarm. You 
...
ping awakens you if youâ€™re asleep.
Alter Self
...
water and gain a Swim Speed equal to your Speed.
 Change Appearance. You alter your appear-
...
statistics change. You canâ€™t appear as a creature of a 
different size, and your basic shape stays the same; 
if youâ€™re bipedal, you canâ€™t use this spell to become 
...
targetâ€™s Challenge Rating isnâ€™t 0, it automatically 
succeeds). You specify a location you have visited 
and a recipient who matches a general descrip-
tion, such as â€œa person dressed in the uniform of 
the town guardâ€ or â€œa red-haired dwarf wearing a 
pointed hat.â€ You also communicate a message of 
...
communication. If the Beast doesnâ€™t reach its desti-
nation before the spell ends, the message is lost, and 
the Beast returns to where you cast the spell.
 Using a Higher-Level Spell Slot. The spellâ€™s dura-
...
take a Magic action to transform the targets again.
    A targetâ€™s game statistics are replaced by the 
chosen Beastâ€™s statistics, but the target retains its 
creature type; Hit Points; Hit Point Dice; alignment; 
ability to communicate; and Intelligence, Wisdom, 
and Charisma scores. The targetâ€™s actions are lim-
ited by the Beast formâ€™s anatomy, and it canâ€™t cast 
spells. The targetâ€™s equipment melds into the new 
form, and the target canâ€™t use any of that equipment 
while in that form.
...
or a Zombie if you chose a corpse (see â€œMonstersâ€ 
for the stat blocks).
...
after which it stops obeying any command youâ€™ve 
...
ber of nonmagical objects within range that arenâ€™t 
being worn or carried, arenâ€™t fixed to a surface, and 
arenâ€™t Gargantuan. The maximum number of objects 
...
ing damage carries over to that form. 
 Using a Higher-Level Spell Slot. The creatureâ€™s 
...
those things canâ€™t target or otherwise affect any-
thing inside it. Magical properties of magic items 
donâ€™t work inside the aura or on anything inside it.
    Areas of effect created by spells or other magic 
canâ€™t extend into the aura, and no one can teleport 
...
is suppressed, it doesnâ€™t function, but the time it 
spends suppressed counts against its duration.
 Dispel Magic has no effect on the aura, and the au-
ras created by different Antimagic Field spells donâ€™t 
nullify each other.
Antipathy/Sympathy
...
save:
A nt ipat hy. The creature has the Frightened condi-
...
is within 5 feet of the target, the creature canâ€™t 
...
Str 16+3+3    dex 10+0+0con 10+0+0
int 3âˆ’4 âˆ’4WiS 3âˆ’4 âˆ’4chA 1âˆ’5 âˆ’5
...
feet in any direction. A solid barrier blocks the eyeâ€™s 
...
to 0 Hit Points, the spell ends. The hand doesnâ€™t oc-
cupy its space.
...
lock canâ€™t be unlocked by any nonmagical means. 
...
ture or an object that isnâ€™t being worn or carried. A 
...
than the targetâ€™s actual type. Spells and other magi-
cal effects treat the target as if it were a creature of 
the chosen type.
    False Aura (Object). You change the way the tar-
...
item appear nonmagical, or change the objectâ€™s aura 
...
Casting Time: 1 hour
Range: 10 feet
Components: V, S, M (for each of the spellâ€™s targets, 
...
plane). Each targetâ€™s body is left behind in a state of 
suspended animation; it has the Unconscious condi-
tion, doesnâ€™t need food or air, and doesnâ€™t age.
    A targetâ€™s astral form resembles its body in almost 
...
after 1 foot. If the cord is cutâ€”which happens only 
when an effect states that it does soâ€”the targetâ€™s 
body and astral form both die.
    A targetâ€™s astral form can travel through the As-
tral Plane. The moment an astral form leaves that 
plane, the targetâ€™s body and possessions travel 
...
tral form have no effect on the targetâ€™s body and 
vice versa. If a targetâ€™s body or astral form drops to 
0 Hit Points, the spell ends for that target. The spell 
ends for all the targets if you take a Magic action to 
dismiss it.
    When the spell ends for a target who isnâ€™t dead, 
...
The spell doesnâ€™t account for circumstances, such as 
...
Hit Point maximums canâ€™t be reduced. If an ally with 
...
natural plant that isnâ€™t a creature. The target gains 
...
mental, a Fey, or a Fiend, the target doesnâ€™t return 
if the spell lasts for 1 minute. The target is instead 
transported to a random location on a plane (GMâ€™s 
...
the targetâ€™s skin assumes a bark-like appearance, 
...
damage and canâ€™t cast spells or take the Magic ac-
...
â€¢ Choose one ability. The target has Disadvantage 
on ability checks and saving throws made with 
that ability.
â€¢ The target has Disadvantage on attack rolls 
against you.
â€¢ In combat, the target must succeed on a Wisdom 
saving throw at the start of each of its turns or be 
forced to take the Dodge action on that turn.

System Reference Document 5.2
113
â€¢ If you deal damage to the target with an attack 
...
a level 5+ spell slot, the spell doesnâ€™t require Con-
centration, and the duration becomes 8 hours (level 
5â€“6 slot) or 24 hours (level 7â€“8 slot). If you use a 
...
fails the save.
    Alternatively, target a nonmagical plant that isnâ€™t 
a creature, such as a tree or shrub. It doesnâ€™t make a 
...
duration. On a roll of 4â€“6, you vanish from your cur-
...
of gray, but you canâ€™t see anything there more than 
60 feet away. You can affect and be affected only 
by other creatures on the Ethereal Plane, and crea-
tures on the other plane canâ€™t perceive you unless 
...
half as much damage on a successful one.
    Flammable objects in the Cone that arenâ€™t being 
...
â€¢ The creature has Immunity to the Charmed and 
...
â€¢ The creature becomes Indifferent about crea-
tures of your choice that itâ€™s Hostile toward. This 
indifference ends if the target takes damage or 
witnesses its allies taking damage. When the spell 
ends, the creatureâ€™s attitude returns to normal.
Chain Lightning
...
target takes 1d10 Necrotic damage, and it canâ€™t re-
...
roll. The orb canâ€™t leap again unless you cast the 
...
inside the vessel used in the spellâ€™s casting and fin-
...
ishes forming, the creatureâ€™s soul transfers to the 
...
none of the originalâ€™s equipment. The creatureâ€™s 
original remains, if any, become inert and canâ€™t be 
revived, since the creatureâ€™s soul is elsewhere.
Cloudkill
...
ends its turn.
Halt. On its turn, the target doesnâ€™t move and takes 
...
question.
    Divine beings arenâ€™t necessarily omniscient, so 
you might receive â€œunclearâ€ as an answer if a ques-
tion pertains to information that lies beyond the de-
ityâ€™s knowledge. In a case where a one-word answer 
could be misleading or contrary to the deityâ€™s inter-
...
System Reference Document 5.2
117
doesnâ€™t function where nature has been replaced by 
construction, such as in castles and settlements.
    Choose three of the following facts; you learn 
those facts as they pertain to the spellâ€™s area:
â€¢ Locations of settlements
â€¢ Locations of portals to other planes of existence
â€¢ Location of one Challenge Rating 10+ creature 
(GMâ€™s choice) that is a Celestial, an Elemental, a 
Fey, a Fiend, or an Undead
â€¢ The most prevalent kind of plant, mineral, or 
Beast (you choose which to learn)
â€¢ Locations of bodies of water
...
one page of text. This spell doesnâ€™t decode symbols 
or secret messages.
Compulsion
...
a Wisdom saving throw, or that target canâ€™t take Bo-
...
1The target doesnâ€™t take an action, and it uses all 
its movement to move. Roll 1d4 for the direc-
tion: 1, north; 2, east; 3, south; or 4, west.
2â€“6The target doesnâ€™t move or take actions.
7â€“8The target doesnâ€™t move, and it takes the At-
...
for the duration, and you choose the spiritsâ€™ animal 
form, such as wolves, serpents, or birds.
    You have Advantage on Strength saving throws 
while youâ€™re within 5 feet of the pack, and when you 
...
within range. Choose the spiritâ€™s element, which 
...
itâ€™s space or starts its turn within 5 feet of the spirit, 
...
spiritâ€™s type, and the target has the Restrained 
...
spiritâ€™s type. On a successful save, the target isnâ€™t 
...
Bonus Action for the spellâ€™s duration.
    Using a Higher-Level Spell Slot. The damage in-
creases by 1d8 for each spell slot level above 4.
Contact Other Plane
...
one word, such as â€œyes,â€ â€œno,â€ â€œmaybe,â€ â€œnever,â€ â€œir-
relevant,â€ or â€œunclearâ€ (if the entity doesnâ€™t know 
...
Poisoned condition doesnâ€™t end on it.
Contingency
...
target you. You cast that spellâ€”called the contin-
gent spellâ€”as part of casting Contingency, expend-
ing spell slots for both, but the contingent spell 
doesnâ€™t come into effect. Instead, it takes effect 
when a certain trigger occurs. You describe that 
trigger when you cast the two spells. For example, 
a Contingency cast with Water Breathing might stip-
...
or smaller vehicles in the waveâ€™s path are carried 
...
ate a trench. The trench extends across the spellâ€™s 
...
spellâ€™s area, it resumes its flow based on the terrain. 
...
and wind. It takes 1d4 Ã— 10 minutes for the new 
...
with a spell slot, the slot isnâ€™t expended.
...
rangeâ€”both useful in fending off the hazards of 
...
Des t roy Water. You destroy up to 10 gallons of 
...
control (see â€œMonstersâ€ for its stat block).
...
See â€œMonstersâ€ for these stat blocks.
Creation
...
a form and material that you have seen.
    The spellâ€™s duration depends on the objectâ€™s ma-
...
another spellâ€™s Material component causes the other 
spell to fail.
Materials
MaterialDuration
...
Adamantine or mithral1 minute
    Using a Higher-Level Spell Slot. The Cube in-
...
and a light vanishes if it exceeds the spellâ€™s range.
Darkness
...
Darkvision canâ€™t see through it, and nonmagical 
light canâ€™t illuminate it.
    Alternatively, you cast the spell on an object that 
isnâ€™t being worn or carried, causing the Darkness 
...
such as a bowl or helm, blocks the Darkness.
    If any of this spellâ€™s area overlaps with an area of 
...
sunlightâ€™s area is Bright Light and sheds Dim Light 
for an additional 60 feet.
    Alternatively, you cast the spell on an object that 
isnâ€™t being worn or carried, causing the sunlight 
...
half as much damage on a successful one.
    The spellâ€™s base damage is 12d6, and the damage 
...
enters a creatureâ€™s space or collides with a solid ob-
ject, the spell ends, and the bead explodes.
  When the bead explodes, flammable objects in the 
explosion that arenâ€™t being worn or carried start 
burning.
...
closest to the doorâ€™s former space.
...
ated by a spell, you learn the spellâ€™s school of magic.
    The spell is blocked by 1 foot of stone, dirt, or 
wood; 1 inch of metal; or a thin sheet of lead.
...
You donâ€™t read the thoughts, but you know that a 
...
getâ€™s mind right now. If the target doesnâ€™t know any 
languages and isnâ€™t telepathic, you learn nothing.
    As a Magic action on your next turn, you can try 
to probe deeper into the targetâ€™s mind. If you probe 
deeper, the target makes a Wisdom saving throw. 
On a failed save, you discern the targetâ€™s reasoning, 
...
shift your attention away from the targetâ€™s mind, 
...
by stating distance and direction, such as â€œ200 feet 
straight downwardâ€ or â€œ300 feet upward to the 
northwest at a 45-degree angle.â€
...
Components: V, S
Duration: 1 hour
You make yourselfâ€”including your clothing, armor, 
weapons, and other belongings on your personâ€”
...
home plane if it isnâ€™t there already. If they arenâ€™t on 
...
your spellcasting ability (DC 10 plus that spellâ€™s 
level). On a successful check, the spell ends.
 Using a Higher-Level Spell Slot. You automati-
cally end a spell on the target if the spellâ€™s level is 
...
spell consumes)
Duration: Instantaneous
This spell puts you in contact with a god or a godâ€™s 
...
or cryptic rhyme. The spell doesnâ€™t account for cir-
...
origin (if it isnâ€™t there already) and canâ€™t return to 
the current plane for 24 hours by any means short 
of a Wish spell.
Divine Word Effects
Hit PointsEffect 
0â€“20The target dies.
21â€“30The target has the Blinded, Deafened, and 
Stunned conditions for 1 hour.
31â€“40The target has the Blinded and Deafened 
conditions for 10 minutes.
41â€“50The target has the Deafened condition for 
1 minute.
Dominate Beast
...
such as â€œAttack that creature,â€ â€œMove over there,â€ or 
â€œFetch that object.â€ The target does its best to obey 
on its turn. If it completes an order and doesnâ€™t re-
...
the targetâ€™s dreams and can converse with the tar-
get as long as it remains asleep, through the spellâ€™s 
duration. The messenger can also shape the dreamâ€™s 
...
of the following effects within range.
 Weather Sensor. You create a Tiny, harmless sen-
...
seed pod open, or a leaf bud bloom.
 Sensory Effect. You create a harmless sensory ef-
...
and its Concentration is broken.
 ...
 Fissures. A total of 1d6 fissures open in the spellâ€™s 
area at the end of the turn you cast it. You choose 
the fissuresâ€™ locations, which canâ€™t be under struc-
tures. Each fissure is 1d10 Ã— 10 feet deep and 10 
feet wide, and it extends from one edge of the spellâ€™s 
...
moves with the fissureâ€™s edge as it opens.
...
structure equal to half the structureâ€™s height makes 
a Dexterity saving throw. On a failed save, the crea-
...
smokeâ€™s scent lingers for 1 minute.
...
diately after it hits or misses a target.
    Enlarge. The targetâ€™s size increases by one cat-
egoryâ€”from Medium to Large, for example. The 
target also has Advantage on Strength checks and 
Strength saving throws. The targetâ€™s attacks with 
its enlarged weapons or Unarmed Strikes deal an 
extra 1d4 damage on a hit.
    Reduce. The targetâ€™s size decreases by one cate-
goryâ€”from Medium to Small, for example. The tar-
get also has Disadvantage on Strength checks and 
Strength saving throws. The targetâ€™s attacks with 
its reduced weapons or Unarmed Strikes deal 1d4 
less damage on a hit (this canâ€™t reduce the damage 
below 1).
Ensnaring Strike
...
has a âˆ’10 penalty to Wisdom (Perception) checks 
...
which looks gray, and you canâ€™t see anything there 
more than 60 feet away.
    While on the Ethereal Plane, you can affect and 
be affected only by creatures, objects, and effects 
on that plane. Creatures that arenâ€™t on the Ethereal 
Plane canâ€™t perceive or interact with you unless a 
...
are on the Ethereal Plane or a plane that doesnâ€™t 
...
canâ€™t target a creature again if it has succeeded on a 
...
where it canâ€™t see you, this effect ends.
...
rial. If youâ€™re working with metal, stone, or another 
...
based on the quality of the raw materials.
    Creatures and magic items canâ€™t be created by 
this spell. You also canâ€™t use it to create items that 
require a high degree of skillâ€”such as weapons and 
armorâ€”unless you have proficiency with the type 
of Artisanâ€™s Tools used to craft such objects.
Faerie Fire
...
dius and canâ€™t benefit from the Invisible condition.
...
creature ends its turn in a space where it doesnâ€™t 
...
falling creatureâ€™s rate of descent slows to 60 feet per 
...
â€œMonstersâ€), though it is a Celestial, Fey, or Fiend 
...
can see through the familiarâ€™s eyes and hear what it 
...
turn. A familiar canâ€™t attack, but it can take other 
actions as normal.
...
wearing or carrying.
 One Familiar Only. You canâ€™t have more than one 
...
steedâ€™s creature typeâ€”Celestial, Fey, or Fiendâ€”
which determines certain traits in the stat block.
 Combat. The steed is an ally to you and your al-
...
pendently, focusing on protecting you.
 Disappearance of the Steed. The steed disap-
...
slotâ€™s level for the spellâ€™s level in the stat block.
...
mechanical pit trap, but it wouldnâ€™t reveal a natural 
...
tion saving throw, taking 7d8 + 30 Necrotic damage 
on a failed save or half as much damage on a suc-
cessful one.
    A Humanoid killed by this spell rises at the start 
of your next turn as a Zombie (see â€œMonstersâ€) that 
...
on a failed save or half as much damage on a suc-
cessful one.
    Flammable objects in the area that arenâ€™t being 
worn or carried start burning.
    Using a Higher-Level Spell Slot. The damage in-
...
Dice [d10s] equal to the spellâ€™s level)
Speed 60 ft., Fly 60 ft. (requires level 4+ spell)
MODSAVEMODSAVEMODSAVE
Str 18+4 +4    dex 12+1+1con 14+2+2
int 6âˆ’2 âˆ’2WiS 12+1+1chA 8âˆ’1  âˆ’1
...
number of Hit Points equal to 2d8 plus the spellâ€™s level.
Find the Path
Level 6 Divination (Bard, Cleric, Druid)
Casting Time: 1 minute
Range: Self
Components: V, S, M (a set of divination toolsâ€”such as 
cards or runesâ€”worth 100+ GP)
...
destination (such as â€œa green dragonâ€™s lairâ€).
...
it isnâ€™t being worn or carried.
 Cantrip Upgrade. The damage increases by 1d10 
when you reach levels 5 (2d10), 11 (3d10), and 17 
(4 d10).
Fire Shield
...
erupts with flame. The attacker takes 2d8 Fire dam-
...
above. Each creature in a 10-foot-radius, 40-foot-
...
sphere into a creatureâ€™s space, that creature makes 
...
to 10 feet wide. Flammable objects that arenâ€™t be-
...
the duration. The successes and failures neednâ€™t be 
...
fied until the condition is ended by Greater Resto-
...
stairs, slopes and the like, but it canâ€™t cross an ele-
vation change of 10 feet or more. For example, the 
disk canâ€™t move across a 10-foot-deep pit, nor could 
it leave such a pit if it was created at the bottom.
    If you move more than 100 feet from the disk (typ-
ically because it canâ€™t move around an obstacle to 
...
perses it.
    Using a Higher-Level Spell Slot. The fogâ€™s radius 
...
tects up to 40,000 square feet of floor space to a 
height of 30 feet above the floor. For the duration, 
creatures canâ€™t teleport into the area or use portals, 
...
of a chosen type enters the spellâ€™s area for the first 
...
ters the area takes no damage from the spell.
    The spellâ€™s area canâ€™t overlap with the area of 
...
pletely inside the cageâ€™s area is trapped. Creatures 
...
area until they are completely outside it.
    A creature inside the cage canâ€™t leave it by non-
...
cage. On a failed save, the creature doesnâ€™t exit the 
...
the targetâ€™s movement is unaffected by Difficult 
Terrain, and spells and other magical effects can 
neither reduce the targetâ€™s Speed nor cause the tar-
...
pleting the spellâ€™s casting. If you do so, a globe about 
...
40 feet) or hurl it with a sling (to the slingâ€™s normal 
...
the globe hasnâ€™t already shattered, it explodes.
    Using a Higher-Level Spell Slot. The damage in-
creases by 1d6 for each spell slot level above 6.
Gaseous Form
Level 3 Transmutation (Sorcerer, Warlock, Wizard)
Casting Time: Action
Range: Tou c h
...
with everything itâ€™s wearing and carrying, into a 
...
System Reference Document 5.2
135
    While in this form, the targetâ€™s only method 
...
surfaces.
    The target canâ€™t talk or manipulate objects, and 
any objects it was carrying or holding canâ€™t be 
dropped, used, or otherwise interacted with. Fi-
nally, the target canâ€™t attack or cast spells.
 Using a Higher-Level Spell Slot. You can target 
one additional creature for each spell slot level 
above 3.
Gate
...
portalâ€™s destination is visible through it.
...
name doesnâ€™t work). If that creature is on a plane 
...
ceeds if it canâ€™t understand your command.
...
tion, the target is protected from decay and canâ€™t 
become Undead.
    The spell also effectively extends the time limit on 
raising the target from the dead, since days spent 
under the influence of this spell donâ€™t count against 
...
you). If you donâ€™t issue any, it takes the Dodge action 
...
AC 11 + the spellâ€™s level
HP 30 + 10 for each spell level above 4
Speed 40 ft., Climb 40 ft., Fly 40 ft. (Wasp only)
MODSAVEMODSAVEMODSAVE
Str 17+3+3    dex 13+1+1con 15+2+2
int 4 âˆ’3 âˆ’3WiS 14+2+2chA 3âˆ’4 âˆ’4
...
equal to half this spellâ€™s level (round down).
Poison Jab. Melee Attack Roll: Bonus equals your spell 
attack modifier, reach 10 ft. Hit: 1d6 + 3 plus the spellâ€™s 
...
+ 3 plus the spellâ€™s level Bludgeoning damage, and the 
targetâ€™s Speed is reduced to 0 until the start of the in-
sectâ€™s next turn.
Bonus Actions
...
condition until the start of the insectâ€™s next turn.
Glibness
...
barrier canâ€™t affect anything within it. Such a spell 
...
and choose whether itâ€™s an explosive rune or a spell 
...
conditions for creatures that donâ€™t trigger the glyph, 
such as those who say a certain password.
    Explosive Rune. When triggered, the glyph erupts 
with magical energy in a 20-foot-radius Sphere cen-
...
â€¢ 1 Exhaustion level
â€¢ The Charmed or Petrified condition
â€¢ A curse, including the targetâ€™s Attunement to a 
cursed magic item
â€¢ Any reduction to one of the targetâ€™s ability scores
â€¢ Any reduction to the targetâ€™s Hit Point maximum
Guardian of Faith
...
als that are unaffected by the spellâ€™s effects. You can 
...
locked, as if sealed by the Arcane Lock spell. In addi-
...
Guards and Wards lasts.
    Other Spell Effect. Place one of the following mag-
ical effects within the warded area:
â€¢ Dancing Lights in four corridors, with a simple 
program that the lights repeat as long as Guards 
and Wards lasts
â€¢ Magic Mouth in two locations
â€¢ Stinking Cloud in two locations (the vapors return 
within 10 minutes if dispersed while Guards and 
Wards lasts)
â€¢ Gust of Wind in one corridor or room (the wind 
blows continuously while the spell lasts)
â€¢ Suggestion in one 5-foot square; any creature 
...
an area already under the effect of Hallow. The af-
...
lower than this spell, canâ€™t illuminate the area.

System Reference Document 5.2
139
Daylight. Bright light fills the area. Magical Dark-
ness created by spells of a level lower than this 
spell canâ€™t extinguish the light.
Peaceful Rest. Dead bodies interred in the area 
canâ€™t be turned into Undead.
Extradimensional Interference. Creatures of any 
types you choose canâ€™t enter or exit the area using 
...
Tong ues. Creatures of any types you choose can 
communicate with any other creature in the area 
even if they donâ€™t share a common language.
...
and creatures within the area arenâ€™t changed.
    The tactile characteristics of the terrain are un-
changed, so creatures entering the area are likely 
to notice the illusion. If the difference isnâ€™t obvious 
...
damage only. This spell canâ€™t reduce a targetâ€™s Hit 
...
range. Until the spell ends, the targetâ€™s Speed is 
...
object if it can. If it doesnâ€™t drop the object, it has 
...
4 hours), 3â€“4 (up to 8 hours), or 5+ (24 hours).
Hideous Laughter
...
trollably if itâ€™s capable of laughter, and it canâ€™t end 
...
to a new creature you can see within range.
 Using a Higher-Level Spell Slot. Your Concentra-
tion can last longer with a spell slot of level 3â€“4 (up 
...
Duration: Instantaneous
You touch an object throughout the spellâ€™s casting. 
...
doesnâ€™t need to breathe, eat, or drink, and it doesnâ€™t 
age. Divination spells canâ€™t locate or perceive the 
imprisoned target, and the target canâ€™t teleport.
...
condition and canâ€™t be moved by any means.
Hedged Prison. The target is trapped in a demi-
...
S lumber. The target has the Unconscious condition 
and canâ€™t be awoken.
...
range. The cloudâ€™s area is Heavily Obscured. It lasts 
...
System Reference Document 5.2
143
    The cloud moves 10 feet away from you in a direc-
...
this save when it enters the spellâ€™s area for the first 
...
objectâ€™s name on the sapphire. Each time you cast 
this spell, you must use a different sapphire.
    Thereafter, you can take a Magic action to speak 
the objectâ€™s name and crush the sapphire. The ob-
...
ject, crushing the sapphire doesnâ€™t transport it, but 
...
Casting Time: Bonus Action
Range: Tou c h
Component: V, S, M (a grasshopperâ€™s hind leg)
Duration: 1 minute
...
tive language or poetry, as determined by the GM.
    If the famous thing you chose isnâ€™t actually fa-
...
were climbing. You can change the targetâ€™s altitude 
...
Duration: 1 hour
You touch one Large or smaller object that isnâ€™t 
...
terity saving throw, taking 8d6 Lightning damage 
on a failed save or half as much damage on a suc-
cessful one.
    Using a Higher-Level Spell Slot. The damage in-
creases by 1d6 for each spell slot level above 3.
...
You sense the direction to the creatureâ€™s location 
...
creature up closeâ€”within 30 feetâ€”at least once. 
If the creature you described or named is in a dif-
ferent form, such as under the effects of a Flesh to 
Stone or Polymorph spell, this spell doesnâ€™t locate 
the creature.
    This spell canâ€™t locate a creature if any thickness 
...
You sense the direction to the objectâ€™s location if 
...
if you have seen it up closeâ€”within 30 feetâ€”at least 
...
apparel, jewelry, furniture, tool, or weapon.
    This spell canâ€™t locate an object if any thickness 
of lead blocks a direct path between you and the 
object.
Longstrider
...
Duration: 1 hour
You touch a creature. The targetâ€™s Speed increases 
...
Duration: 8 hours
You touch a willing creature who isnâ€™t wearing 
armor. Until the spell ends, the targetâ€™s base AC be-
comes 13 plus its Dexterity modifier. The spell ends 
...
tion. The hand vanishes if it is ever more than 30 
...
can move the hand up to 30 feet.
    The hand canâ€™t attack, activate magic items, or 
...
the following ways:
â€¢ The creature canâ€™t willingly enter the Cylinder 
...
â€¢ The creature has Disadvantage on attack rolls 
against targets within the Cylinder.

System Reference Document 5.2
146
â€¢ Targets within the Cylinder canâ€™t be possessed 
...
spellâ€™s Material component. While your soul inhab-
its the container, you are aware of your surround-
ings as if you were in the containerâ€™s space. You 
canâ€™t move or take Reactions. The only action you 
...
spell canâ€™t be possessed). The target makes a Cha-
risma saving throw. On a failed save, your soul en-
ters the targetâ€™s body, and the targetâ€™s soul becomes 
trapped in the container. On a successful save, the 
target resists your efforts to possess it, and you 
canâ€™t attempt to possess it again for 24 hours.
    Once you possess a creatureâ€™s body, you control it. 
Your Hit Points, Hit Point Dice, Strength, Dexterity, 
Constitution, Speed, and senses are replaced by the 
creatureâ€™s. You otherwise keep your game statistics.
    Meanwhile, the possessed creatureâ€™s soul can per-
ceive from the container using its own senses, but it 
canâ€™t move and it is Incapacitated.
...
tureâ€™s soul to its body. If the host body dies while 
youâ€™re in it, the creature dies, and you make a Cha-
...
you die. If another creatureâ€™s soul is in the container 
when it is destroyed, the creatureâ€™s soul returns to 
...
the spell consumes)
Duration: Until dispelled
You implant a message within an object in rangeâ€”a 
message that is uttered when a trigger condition is 
met. Choose an object that you can see and that isnâ€™t 
...
appear to come from the objectâ€™s mouth. When you 
...
creases to +2 with a level 3â€“5 spell slot. The bonus 
...
sional dwelling and is 5 feet wide and 10 feet tall. 
...
canâ€™t attack or take any action that would directly 
harm another creature. Thus the servants can fetch 
things, clean, mend, fold clothes, light fires, serve 
food, pour wine, and so on. The servants canâ€™t leave 
the dwelling.
...
it canâ€™t deal damage or cause conditions.
...
Casting Time: Action
Range: 60 feet
Components: V, M (a snakeâ€™s tongue)
Duration: 24 hours
You suggest a course of activityâ€”described in no 
more than 25 wordsâ€”to twelve or fewer creatures 
...
example, you could say, â€œWalk to the village down 
that road, and help the villagers there harvest crops 
until sunset.â€ Or you could say, â€œNow is not the time 
...
cape. When it does so, it makes a DC 20 Intelligence 
...
nonmagical senses.
    While merged with the stone, you canâ€™t see what 
...
the spell. You otherwise canâ€™t move.
    Minor physical damage to the stone doesnâ€™t harm 
...
stoneâ€™s complete destruction (or transmutation into 
...
it canâ€™t restore magic to such an object.
Message
...
once.
    A nonmagical object that isnâ€™t being worn or car-
ried also takes the damage if itâ€™s in the spellâ€™s area, 
and the object starts burning if itâ€™s flammable.
Mind Blank
...
no spellâ€”not even Wishâ€”can gather information 
...
know the targetâ€™s location until the spell ends, but 
only while the two of you are on the same plane 
of existence. While you have this knowledge, the 
target canâ€™t become hidden from you, and if it has 
...
someone elseâ€™s voice, a lionâ€™s roar, a beating of 
...
the spell ends.
 Image. If you create an image of an objectâ€”such 
as a chair, muddy footprints, or a small chestâ€”it 
must be no larger than a 5-foot Cube. The image 
canâ€™t create sound, light, smell, or any other sensory 
...
doesnâ€™t disguise, conceal, or add creatures.
...
that is removed from the spellâ€™s area disappears 
immediately.
    Creatures with Truesight can see through the illu-
sion to the terrainâ€™s true form; however, all other el-
ements of the illusion remain, so while the creature 
is aware of the illusionâ€™s presence, the creature can 
...
you and mimic your actions, shifting position so itâ€™s 
impossible to track which image is real.
    Each time a creature hits you with an attack roll 
during the spellâ€™s duration, roll a d6 for each of 
...
Components: V, S
Duration: Concentration, up to 1 minute
You attempt to reshape another creatureâ€™s mem-
...
spell, this spell ends, and no memories are modified.
    While this charm lasts, you can affect the targetâ€™s 
...
perfect clarity, change its memory of the eventâ€™s de-
...
describing the modified memories, the creatureâ€™s 
memory isnâ€™t altered. Otherwise, the modified 
memories take hold when the spell ends.
  A modified memory doesnâ€™t necessarily affect 
how a creature behaves, particularly if the mem-
ory contradicts the creatureâ€™s natural inclinations, 
...
on the target restores the creatureâ€™s true memory.
 Using a Higher-Level Spell Slot. You can alter the 
targetâ€™s memories of an event that took place up to 
7 days ago (level 6 spell slot), 30 days ago (level 7 
spell slot), 365 days ago (level 8 spell slot), or any 
time in the creatureâ€™s past (level 9 spell slot).
Moonbeam
...
and canâ€™t shape-shift until it leaves the Cylinder. 
On a successful save, a creature takes half as much 
damage only. A creature also makes this save when 
the spellâ€™s area moves into its space and when it 
enters the spellâ€™s area or ends its turn there. A crea-
ture makes this save only once per turn.
    Using a Higher-Level Spell Slot. The damage in-
creases by 1d10 for each spell slot level above 2.
Move Earth
Level 6 Transmutation (Druid, Sorcerer, Wizard)
Casting Time: Action
Range: 120 feet
...
terrain to affect within range.
    This spell canâ€™t manipulate natural stone or stone 
...
collapse.
    Similarly, this spell doesnâ€™t directly affect plant 
...
than 10 feet in any dimension. The target canâ€™t 
...
tion. You choose the openingâ€™s dimensions: up to 5 
...
Duration: Concentration, up to 1 hour
You radiate a concealing aura in a 30-foot Emana-
tion for the duration. While in the aura, you and 
each creature you choose have a +10 bonus to Dex-
...
phantasm can deal 2d8 Psychic damage to the tar-
get if it is in the phantasmâ€™s area or within 5 feet of 
...
within range. You decide the creatureâ€™s appearance, 
...
stat block (see â€œMonstersâ€), except it has a Speed of 
...
it isnâ€™t obliged to do so. The requested task could 
...
the spell. If the task is aligned with the creatureâ€™s 
...
price for the creatureâ€™s service, the creature imme-
...
created by another spell, that spellâ€™s duration is ex-
...
one or more areas of any size within the spellâ€™s area 
from being affected.
 Enrichment. All plants in a half-mile radius cen-
...
to or less than the targetâ€™s (or the targetâ€™s level if 
it doesnâ€™t have a Challenge Rating). The targetâ€™s 
...
Dice. See the â€œAnimalsâ€ section of â€œMonstersâ€ for a 
...
by the anatomy of its new form, and it canâ€™t speak or 
cast spells.

System Reference Document 5.2
154
    The targetâ€™s gear melds into the new form. The 
creature canâ€™t use or otherwise benefit from any of 
that equipment.
Power Word Heal
...
within range for the spellâ€™s entire casting gain the 
benefits of a Short Rest and also regain 2d8 Hit 
Points. A creature canâ€™t be affected by this spell 
...
Prismatic Rays
1d8Ray
1Red. Failed Save: 12d6 Fire damage. Successful 
Save: Half as much damage.
2Orange. Failed Save: 12d6 Acid damage. Successful 
Save: Half as much damage.
3Yellow. Failed Save: 12d6 Lightning damage. Suc-
cessful Save: Half as much damage.
4Green. Failed Save: 12d6 Poison damage. Success-
ful Save: Half as much damage.

System Reference Document 5.2
155
1d8Ray
5Blue. Failed Save: 12d6 Cold damage. Successful 
Save: Half as much damage.
6Indigo. Failed Save: The target has the Restrained 
...
spell. The successes and failures neednâ€™t be con-
...
plane of existence (GMâ€™s choice).
8Special. The target is struck by two rays. Roll 
...
a vertical opaque wallâ€”up to 90 feet long, 30 feet 
high, and 1 inch thickâ€”centered on a point within 
...
fected by that layerâ€™s properties as described in the 
...
failures neednâ€™t be consecutive; keep track of 
both until the target collects three of a kind. 
Additional Effects: Spells canâ€™t be cast through 
...
to another plane of existence (GMâ€™s choice). 
...
lowing properties:
â€¢ Sound canâ€™t pass through the barrier at the edge 
of the warded area.
â€¢ The barrier of the warded area appears dark and 
foggy, preventing vision (including Darkvision) 
through it.
â€¢ Sensors created by Divination spells canâ€™t appear 
inside the protected area or pass through the bar-
rier at its perimeter.
â€¢ Creatures in the area canâ€™t be targeted by Divina-
tion spells.
â€¢ Nothing can teleport into or out of the warded 
area.
...
takes any damage, it disappears, and the spell ends.
    You can see through the illusionâ€™s eyes and hear 
...
against the target. The target also canâ€™t be pos-
...
been dead no longer than 10 days and it wasnâ€™t Un-
dead when it died.
...
creature at the time of death.
    This spell closes all mortal wounds, but it doesnâ€™t 
restore missing body parts. If the creature is lack-
ing body parts or organs integral for its survivalâ€”
its head, for instanceâ€”the spell automatically fails.
    Coming back from the dead is an ordeal. The tar-
get takes a âˆ’4 penalty to D20 Tests. Every time the 
...
below to determine the bodyâ€™s species, or the GM 
chooses another playable species.
1d10Species1d10Species
1Roll again.6Goliath
2Dragonborn7Halfling
3Dwarf8Human
4Elf9Orc
5Gnome10Tiefling
    The reincarnated creature makes any choices that 
a speciesâ€™ description offers, and the creature re-
...
its curse remains, but the spell breaks its ownerâ€™s 
...
be enclosed for the duration.
  Nothingâ€”not physical objects, energy, or other 
spell effectsâ€”can pass through the barrier, in or 
out, though a creature in the sphere can breathe 
there. The sphere is immune to all damage, and a 
creature or object inside canâ€™t be damaged by at-
...
sphereâ€™s walls and thus roll the sphere at up to half 
the creatureâ€™s Speed. Similarly, the globe can be 
...
been dead for no more than a century, didnâ€™t die of 
old age, and wasnâ€™t Undead when it died.
...
you finish a Long Rest, you canâ€™t cast spells again, 
...
All creatures and objects in that area that arenâ€™t 
...
fected creature or object reaches the Cylinderâ€™s top 
...
spell canâ€™t revive a creature that has died of old age, 
...
rope reaches a ceiling. At the ropeâ€™s upper end, an 
...
creatures. Attacks, spells, and other effects canâ€™t 
...
a Dexterity saving throw or take 1d8 Radiant dam-
...
doesnâ€™t protect the warded creature from areas of 
...
to it. The target doesnâ€™t know what it is making the 
...
Extensive (know the target well)âˆ’5
You Have the Targetâ€™s ...Save Modifier
Picture or other likenessâˆ’2
Garment or other possessionâˆ’4
Body part, lock of hair, or bit of nailâˆ’10
    On a successful save, the target isnâ€™t affected, and 
you canâ€™t use this spell on it again for 24 hours.
...
appears at that location and doesnâ€™t move.
Searing Smite
Level 1 Evocation (Paladin)
Casting Time: Bonus Action, which you take immedi-
ately after hitting a target with a Melee weapon or an 
...
until the spell ends, the target takes 1d6 Fire dam-
...
pearance of the targetsâ€™ bodies and equipment. 
You can make each creature seem 1 foot shorter or 
taller and appear heavier or lighter. A targetâ€™s new 
...
spell to add a hat to a creatureâ€™s outfit, objects pass 
through the hat.
...
chance that the message doesnâ€™t arrive. You know if 
the delivery fails.
...
Invisible condition and canâ€™t be targeted by Divina-
...
doesnâ€™t age, and doesnâ€™t need food, water, or air.
...
Examples include â€œafter 1,000 yearsâ€ or â€œwhen the 
tarrasque awakens.â€ This spell also ends if the tar-
...
seen the sort of creature before, and it canâ€™t be a 
...
and shape to fit the new form while youâ€™re in it. 
Shatter
...
rolls against it have Advantage, and it canâ€™t benefit 
...
target. On a hit, the target takes 1d8 Lightning dam-
age, and it canâ€™t make Opportunity Attacks until the 
...
is purely visual; it isnâ€™t accompanied by sound, 
...
is half as much, and it canâ€™t cast this spell.
    The simulacrum is Friendly to you and creatures 
you designate. It obeys your commands and acts on 
your turn in combat. The simulacrum canâ€™t gain lev-
els, and it canâ€™t take Short or Long Rests.
...
takes an action to shake it out of the spellâ€™s effect.
    Creatures that donâ€™t sleep, such as elves, or that 
...
fected by this spell for the duration.
  An affected targetâ€™s Speed is halved, it takes a âˆ’2 
penalty to AC and Dexterity saving throws, and it 
canâ€™t take Reactions. On its turns, it can take either 
...
target making the spellâ€™s gestures too slowly.
...
can add to the spellâ€™s damage equals your spellcast-
...
and isnâ€™t dead. The creature becomes Stable.
...
the Influence actionâ€™s skill options with them.
    Most Beasts have little to say about topics that 
donâ€™t pertain to survival or companionship, but at 
...
as an enemy. This spell doesnâ€™t return the creatureâ€™s 
soul to its body, only its animating spirit. Thus, the 
corpse canâ€™t learn new information, doesnâ€™t compre-
hend anything that has happened since it died, and 
canâ€™t speculate about future events.
Speak with Plants
Level 3 Transmutation (Bard, Druid, Ranger)
Casting Time: Action
Range: Self
...
events in the spellâ€™s area within the past day, gain-
...
into Difficult Terrain that lasts for the duration.
    The spell doesnâ€™t enable plants to uproot them-
...
to look natural. Any creature that canâ€™t see the area 
...
Components: V, S, M (a prayer scroll)
Duration: Concentration, up to 10 minutes
Protective spirits flit around you in a 15-foot Ema-
...
tures to be unaffected by it. Any other creatureâ€™s 
Speed is halved in the Emanation, and whenever the 
Emanation enters a creatureâ€™s space and whenever 

System Reference Document 5.2
165
a creature enters the Emanation or ends its turn 
there, the creature must make a Wisdom saving 
throw. On a failed save, the creature takes 3d8 Ra-
...
makes this save only once per turn.
    Using a Higher-Level Spell Slot. The damage in-
creases by 1d8 for each spell slot level above 3.
Spiritual Weapon
...
against a creature within 5 feet of it.
 Using a Higher-Level Spell Slot. The damage in-
creases by 1d8 for every slot level above 2.
Starry Wisp
...
Light in a 10-foot radius and canâ€™t benefit from the 
Invisible condition.
 Cantrip Upgrade. The damage increases by 1d8 
when you reach levels 5 (2d8), 11 (3d8), and 17 
(4 d8).
...
canâ€™t take an action or a Bonus Action.
Stone Shape
Level 4 Transmutation (Cleric, Druid, Wizard)
Casting Time: Action
Range: Tou c h
...
a latch, but finer mechanical detail isnâ€™t possible.
Stoneskin
...
System Reference Document 5.2
166
    Turn 4. Hailstones rain down. Each creature un-
der the cloud takes 2d6 Bludgeoning damage.
    Tur ns 5â€“10. Gusts and freezing rain assail the 
...
Duration: Concentration, up to 8 hours
You suggest a course of activityâ€”described in no 
more than 25 wordsâ€”to one creature you can see 
...
say, â€œFetch the key to the cultâ€™s treasure vault, and 
give the key to me.â€ Or you could say, â€œStop fighting, 
leave this library peacefully, and donâ€™t return.â€
...
AC 14 + the spellâ€™s level
HP 50 + 10 for each spell level above 5
Speed 30 ft., Fly 60 ft., Swim 30 ft.
MODSAVEMODSAVEMODSAVE
Str 19+4 +4    dex 14+2+2con 17+3+3
int 10+0+0WiS 14+2+2chA 14+2+2
...
equal to half the spellâ€™s level (round down), and it uses 
Breath Weapon.
Rend. Melee Attack Roll: Bonus equals your spell attack 
modifier, reach 10 feet. Hit: 1d6 + 4 + the spellâ€™s level 
Piercing damage.
...
throw. On a failed save, a creature takes 12d6 Ra-
...
ing throw or have the Unconscious condition for 10 
...
direction within the spellâ€™s range. Until the end of 
...
object. If the object isnâ€™t being worn or carried, you 
automatically move it up to 30 feet in any direction 
within the spellâ€™s range.
...
in any direction within the spellâ€™s range.
...
tion. Creatures that canâ€™t communicate in any lan-
guages arenâ€™t affected by this spell.
...
over any distance, though it canâ€™t extend to other 
planes of existence.
Teleport
...
be Large or smaller, and it canâ€™t be held or carried 
...
come table and the explanations after it.
...
On 
Target
Permanent circleâ€”â€”â€”01â€“ 00
Linked objectâ€”â€”â€”01â€“ 00
Very familiar01â€“0506 â€“1314â€“2425â€“00
Seen casually01â€“3334â€“4344â€“5354â€“00
Viewed once or 
described
01â€“4344â€“5354â€“7374â€“00
False destination01â€“5051â€“00â€”â€”
    Familiarity. Here are the meanings of the terms 
in the tableâ€™s Familiarity column:
â€¢ â€œPermanent circleâ€ means a permanent teleporta-
tion circle whose sigil sequence you know.
â€¢ â€œLinked objectâ€ means you possess an object taken 
from the desired destination within the last six 
months, such as a book from a wizardâ€™s library.
â€¢ â€œVery familiarâ€ is a place you have visited often, 
a place you have carefully studied, or a place you 
can see when you cast the spell.
â€¢ â€œSeen casuallyâ€ is a place you have seen more 
than once but with which you arenâ€™t very familiar.
â€¢ â€œViewed once or describedâ€ is a place you have 
seen once, possibly using magic, or a place you 
know through someone elseâ€™s description, per-
haps from a map.
â€¢ â€œFalse destinationâ€ is a place that doesnâ€™t exist. 
Perhaps you tried to scry an enemyâ€™s sanctum but 
instead viewed an illusion, or you are attempting 
to teleport to a location that no longer exists.
 Mishap. The spellâ€™s unpredictable magic results in 
...
object) appear in a different area thatâ€™s visually or 
...
pear in another personâ€™s laboratory in the same city.
    Of f Target . You and your group (or the target ob-
ject) appear 2d12 miles away from the destination 
...
northwest; 7, north; or 8, northeast.
    On Target . You and your group (or the target ob-
...
Each circle includes a unique sigil sequenceâ€”a 
...
dim, or change color for 1 minute.
 Invisible Hand. You instantaneously cause an un-
...
spell fails when you cast it if the Emanation isnâ€™t big 
...
passing through it. Spells of level 3 or lower canâ€™t be 
cast through it, and the effects of such spells canâ€™t 
extend into it.
...
you choose, but itâ€™s transparent from the inside.
...
out of the tree youâ€™re in. You appear in a spot of your 
...
throw, and if it succeeds, it isnâ€™t affected by this spell.
...
equal to or less than the targetâ€™s Challenge Rating 
or level. The targetâ€™s game statistics are replaced by 
...
any kind of creature, as long as the creatureâ€™s size 
is no larger than the objectâ€™s size and the creature 
...
objectâ€™s size is no larger than the creatureâ€™s size. The 
creatureâ€™s statistics become those of the object, and 
...
System Reference Document 5.2
171
creatureâ€™s name. The creature then appears in an 
...
attack with the weapon used in the spellâ€™s casting. 
...
diant damage or the weaponâ€™s normal damage type 
(your choice).
 Cantrip Upgrade. Whether you deal Radiant dam-
age or the weaponâ€™s normal damage type, the attack 
...
makes a Strength saving throw, taking 6d10 Blud-
...
At the end of the turn, the wallâ€™s height is reduced 
...
Strength of 2, and it canâ€™t attack. If it drops to 0 Hit 
...
damage only.
 Using a Higher-Level Spell Slot. The initial dam-
age increases by 2d4 for each spell slot level above 
4.
Wall of Fire
Level 4 Evocation (Druid, Sorcerer, Wizard)
Casting Time: Action
...
cast this spell, deals 5d8 Fire damage to each crea-
ture that ends its turn within 10 feet of that side or 
...
for the duration.
    If the wall cuts through a creatureâ€™s space when 
...
10-foot section, and it has Immunity to Cold, Poi-
...
by-20-foot panels that are only 3 inches thick.
    If the wall cuts through a creatureâ€™s space when 
it appears, the creature is pushed to one side of the 
...
canâ€™t occupy the same space as a creature or object. 
The wall doesnâ€™t need to be vertical or rest on a firm 
...
connected panels to collapse at the GMâ€™s discretion.
    If you maintain your Concentration on this spell 
for its full duration, the wall becomes permanent 
and canâ€™t be dispelled. Otherwise, the wall disap-
...
surfaceâ€”such as water, acid, mud, snow, quicksand, 
or lavaâ€”as if it were harmless solid ground (crea-
...
pass from the liquidâ€™s surface into the liquid itself 
...
area within them is Lightly Obscured.
    If the webs arenâ€™t anchored between two solid 
...
Components: V, S
Duration: Concentration, up to 1 minute
You try to create illusory terrors in othersâ€™ minds. 
...
1 minute until it lands, which it does safely. If it canâ€™t 
...
objects canâ€™t pass through the wall. Loose, light-
...
unaffected. Creatures in gaseous form canâ€™t pass 
through it.
...
donâ€™t need to meet any requirements to cast that 
...
25,000 GP in value that isnâ€™t a magic item. The 
...
the new one. You canâ€™t replace a feat that is a pre-
...
spell could undo an allyâ€™s failed saving throw or a 
foeâ€™s Critical Hit. You can force the reroll to be made 
...
presence of the itemâ€™s current owner. If your wish 
...
a god, the godâ€™s divine servants might instantly in-
...
canâ€™t be reduced or prevented in any way. In addi-
...
that enters the spellâ€™s area for the first time on a 
turn or starts its turn there makes a Charisma sav-
ing throw. On a failed save, a creature canâ€™t speak a 
